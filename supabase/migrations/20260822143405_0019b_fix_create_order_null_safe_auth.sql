-- Fix NULL-safe authorization check in create_order_from_collaboration
-- The original guard used `<>` which evaluates to NULL when auth.uid() is NULL,
-- bypassing the authorization check entirely for unauthenticated callers.
...
BEGIN
  ...
  -- Authorization: caller must be a participant in this collaboration (NULL-safe)
  IF v_collab.sender_id IS DISTINCT FROM v_caller_id AND v_collab.traveler_id IS DISTINCT FROM v_caller_id THEN
