/*
# Security Hardening: Add authorization check to create_order_from_collaboration

## Issue
The function did NOT verify that auth.uid() is a participant (sender or traveler)
of the collaboration. Any authenticated user who knows a collaboration ID could
call this function to create an order from someone else's collaboration.

Since the function is SECURITY DEFINER, the internal SELECT bypasses RLS,
so the collaboration row is accessible regardless of RLS policies.

## Fix
Add an authorization check after fetching the collaboration:
- auth.uid() must equal v_collab.sender_id OR v_collab.traveler_id
- Otherwise RAISE EXCEPTION

## Also: Use calculate_platform_fee() instead of hardcoded 5%
The old code hardcoded v_platform_fee := LEAST(v_agreed_price * 0.05, 50).
This should use the active fee configuration via calculate_platform_fee().
*/
