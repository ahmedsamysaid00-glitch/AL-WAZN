/*
# Fix get_admin_feedback RPC: Ambiguous Column + submitter_role Type Mismatch

1. Root Causes
- a) Ambiguous column reference: unqualified column names in the final SELECT
     caused error 42702 (column "id" is ambiguous).
- b) Type mismatch: profiles.role is user_role enum but the function
     declares submitter_role as text, causing error 42804.

2. Fix
- Qualify all columns with "filtered." alias.
- Cast submitter_role to text explicitly.
...
BEGIN
  ...
  WITH filtered AS (
    SELECT
      f.id, f.user_id, f.user_type, f.type, f.category,
      f.title, f.message, f.rating, f.status, f.is_public,
      f.is_sensitive, f.source, f.page_url, f.device_type,
      f.operating_system, f.browser, f.context_type, f.context_id,
      f.expected_behavior, f.proposed_solution, f.archived_at,
      f.archived_by, f.created_at, f.updated_at,
