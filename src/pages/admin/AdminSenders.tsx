...
export function AdminSenders() {
  ...
  const fetchUsers = useCallback(async () => {
    setLoading(true);
    setError(false);
    let query = supabase.from('profiles').select('*', { count: 'exact' }).eq('role', 'sender');
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">{t('admin.sendersTitle')}</h1>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">{t('admin.sendersSubtitle')}</p>
        </div>
        ...
        <div className="card overflow-hidden">
          {loading ? (
            ...
            <EmptyState icon={<Package className="h-8 w-8" />} title={hasFilters ? t('admin.noResults') : t('empty.noSenders')} />
          ) : (
            <>
